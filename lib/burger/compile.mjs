...
const renderComponent = (paths, name, $) => {
    ...
    throw new IpsenError(`Unable to load component ${name}`)
}
const compile = (file, current, renderComponent) => {
    const source = fs.read(file)
    if (source === undefined) {
        return null
    }
    const tokens = tokenize(source)
    const basePath = path.dirname(current)
    const extra = {
        escape,
        renderComponent: (name, args) => {
            const target = path.join(basePath, name).replace(/\\/g, "/")
            return renderComponent(target, args)
        },
        current,
